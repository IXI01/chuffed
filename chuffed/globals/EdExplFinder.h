...
public:
     ...
     * @param dpMatrix: a matrix including the shortest edit distance to each position of the dynamic programming matrix
     * @param lb: a valid lowerbound for the edit distance of two sequences that have been used to build the dp matrices
     * @param seqSize: the size of the sequences for which explanation inequalities should be determined
     * @param min_id_cost: minimum insertion/deletion cost to determine which "diagonal" of the matrix should be calculated
     * @return An explanation clause for a propagation on the edit distance lower bound
     */
     Clause * FindEdExplanation(int _max_char, const vec<int> *_insertion_cost, const vec<int> *_deletion_cost, const vec<int> *_substitution_cost, IntView<> *_seq1, IntView<> *_seq2, const vec<int> *_dpMatrix, int _lb, int _seqSize, int _min_id_cost);
    ~EdExplFinder();
